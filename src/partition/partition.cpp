...
void partition(int* A, int size, int index) {
    int pivot = A[index];
    int steps = 0;
...
void partitionOnePass(int* A, int size, int index) {
    int pivot = A[index];
    int smallSteps = 0;
    int j = 0;
    for (int i=0; i < size; i++) {
        smallSteps++;
        if (A[i] < pivot) {
            // printf("A[%lu]: %d, A[%lu]: %d\n", i, A[i], j, A[j]);
            std::swap(A[i], A[j++]);
        }
    }
    int largeSteps = 0;
    j = size - 1;
    for (int i=j; i >= 0 and A[i] >= pivot; i--) {
        largeSteps++;
        if (A[i] > pivot) {
            // printf("A[%lu]: %d, A[%lu]: %d\n", i, A[i], j, A[j]);
            std::swap(A[i], A[j--]);
    ...
    printf("%-10s smaller-than: %d\n"
           "%10s larger-than: %d\n"
           "%10s total: %d\n", "steps ->", smallSteps, "", largeSteps, "", smallSteps + largeSteps);
}

/**
 * experimental, doesn't work
 */
void partitionFast(int* A, int size, int index) {
    int pivot = A[index];
    int steps = 0;
    int start = 0;
    int end = size - 1;
...
void doPartition(int* A, int size, int index) {
